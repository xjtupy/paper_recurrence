...
def get_embedding(counter, data_type, limit=-1, emb_file=None, size=None, vec_size=None, token2idx_dict=None):
    ...
    if emb_file is not None:
        ...
        print(
            "{} / {} tokens have corresponding {} embedding vector".format(len(embedding_dict), len(filtered_elements),
                                                                           data_type))
    else:
        assert vec_size is not None
    ...
    token2idx_dict = {token: idx for idx, token in
                      enumerate(embedding_dict.keys(), 2)} if token2idx_dict is None else token2idx_dict
...
def build_features(config, examples, data_type, word2idx_dict, char2idx_dict, is_test=False):
    ...
    for example in tqdm(examples):
        ...
        record = {
            "context_idxs": context_idxs.tolist(),
            "ques_idxs": ques_idxs.tolist(),
            "context_char_idxs": context_char_idxs.tolist(),
            "ques_char_idxs": ques_char_idxs.tolist(),
            "y1": y1.tolist(),
            "y2": y2.tolist(),
            "id": example["id"]
        }
...
def save(filename, obj, message=None):
    if message is not None:
        ...
        with open(filename, "w") as fh:
            json.dump(obj, fh)
        print("Saving {}...end".format(message))
...
def prepro(config):
    ...
    train_data, _ = build_features(config, train_examples, "train", word2idx_dict, char2idx_dict)
    dev_data, dev_meta = build_features(config, dev_examples, "dev", word2idx_dict, char2idx_dict)
    test_data, test_meta = build_features(config, test_examples, "test", word2idx_dict, char2idx_dict, is_test=True)

    save(config.word_emb_file, word_emb_mat, message="word embedding")
    save(config.char_emb_file, char_emb_mat, message="char embedding")
    save(config.train_eval_file, train_eval, message="train eval")
    save(config.train_data_file, train_data, message="train data")
